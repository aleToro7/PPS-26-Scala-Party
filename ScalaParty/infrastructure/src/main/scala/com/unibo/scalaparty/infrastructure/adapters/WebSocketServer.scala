...
class WebSocketServer(
 ...
 commandPort: GameCommandPort[IO]
  ...
  def onConnect(playerId: PlayerId): IO[MatchId] = for {
    matchId <- commandPort.joinLobby(playerId)
    _       <- connections.bindSessionToMatch(playerId, matchId)
    _       <- IO.println(s"Player $playerId assigned to the match $matchId")
  } yield matchId

  def onDisconnect(matchId: MatchId, playerId: PlayerId): IO[Unit] = for {
    _ <- commandPort.leaveLobby(matchId, playerId)
    _ <- connections.removeSession(playerId)
    - <- IO.println(s"Player $playerId disconnected")
  } yield ()
