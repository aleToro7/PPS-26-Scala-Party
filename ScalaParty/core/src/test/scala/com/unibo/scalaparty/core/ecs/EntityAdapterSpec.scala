...
import com.unibo.scalaparty.core.dto.{EntityAdapter, toDto}
...
class EntityAdapterSpec extends AnyFlatSpec with Matchers:
  ...
  "EntityAdapter" should "convert a spaceship entity to its corresponding DTO" in:
    ...
    val components = List(PositionComponent(position), MovementComponent(velocity), EntityTypeComponent(EntityType.Spaceship))
